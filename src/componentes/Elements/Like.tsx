...
import { Favorito, LikeProps } from '../../types/types';

{
    // rgb(189, 91, 255)
}
const Like: React.FC<LikeProps> = ({ cocheId }) => {
  const [liked, setLiked] = useState(false);

    useEffect(() => {
    const usuarioId = localStorage.getItem('id');
    const favoritosRaw = sessionStorage.getItem('favoritos');
    if (usuarioId && favoritosRaw) {
      const favoritos = JSON.parse(favoritosRaw);
      const existe = favoritos.some(
        (f: Favorito) => f.usuarioId === parseInt(usuarioId) && f.cocheId === cocheId
      );
      setLiked(existe);
    }
  }, [cocheId]);
  ...
  const handleLikeChange = () => {
    ...
    const index = favoritos.findIndex(
      (f: any) => f.usuarioId === parseInt(usuarioId) && f.cocheId === cocheId
    );

    if (index === -1) {  // si vale -1 no se encontró el favorito en la lista
      favoritos.push({ usuarioId: parseInt(usuarioId), cocheId });
      setLiked(true);
    } else { // si existe el favorito, debe eliminarse 
